...
3. Compile dist files (Development) :
    ...
    * Make your changes only in ./build Folder OR package.json OR ./adminlte/dist/js/demo.js OR package-lock.json OR ./adminlte/dist/js/pages/ OR ./docs OR in any html files which are necessary for contribution
    * Do not make changes in ./adminlte/dist/css/ AND ./adminlte/dist/js/ AND ./plugins Because it contains compiled files and do not include in PR (Pull Request)
